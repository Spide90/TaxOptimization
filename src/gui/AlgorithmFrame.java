...
/**
 * Algorithmn frame for all optimization algorithmns
 * @author chris
 * 
 */
public class AlgorithmFrame extends JFrame {
	...
	public void updatePlots(DataSource... plotsData) {
		...
		plotsPanel.add(createLinePlot(periodsData, "Outcome per period",
				"Period", "Outcome", Color.blue));

		if (algorithmName.equals("Particle Swarm")) {
			plotsPanel.setPreferredSize(new Dimension(
					preferredPlotSize.height * 3, preferredPlotSize.width));
			plotsPanel.setLayout(new GridLayout(3, 1));
			plotsPanel.add(createLinePlot(plotsData[0],
					"Best outcome per iteration", "Iteration", "Best outcome",
					Color.green));
		} else {
	...
	private JPanel createLinePlot(DataSource data, String title,
